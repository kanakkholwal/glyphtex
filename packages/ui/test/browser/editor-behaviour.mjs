...
// Prose blocks are edited in place (see visual-editing.mjs); a block the model
// does not understand still opens its own source instead of pretending.
const opened = await ev(`(() => {
  const chip = document.querySelector('.glyphtex-doc .border-dashed');
  const target = chip?.closest('[role="button"]');
  target?.click();
  return !!target;
})()`);
