...
class SettingsStore {
	...
	get docFontStack(): string {
		return DOC_FONT_STACKS[this.docFont];
	}

	get docSmallText(): boolean {
		return this.#editor.current.docSmallText ?? false;
	}
	set docSmallText(value: boolean) {
		this.patchEditor({ docSmallText: value });
	}

	get docFullWidth(): boolean {
		return this.#editor.current.docFullWidth ?? false;
	}
