...
export const latexLanguage: Monaco.languages.IMonarchLanguage = {
	...
	tokenizer: {
		...
		// `$…\]` must not terminate.
		mathInline: [{ include: '@mathBody' }, [/\$/, { token: 'delimiter.math', next: '@pop' }]],
		...
		mathBracket: [{ include: '@mathBody' }, [/\\\]/, { token: 'delimiter.math', next: '@pop' }]],

		mathParen: [{ include: '@mathBody' }, [/\\\)/, { token: 'delimiter.math', next: '@pop' }]],
