...
export class CompileStore {
	...
	readonly problems = $derived(parseLatexLog(this.compileLog, this.compileError));
	...
	get projectCompile(): boolean {
		return Boolean(this.#compileProject && this.#files.projectRoot && this.#files.mainId);
	}
	get canCompile(): boolean {
		return Boolean(this.#getCompile()) || Boolean(this.#getCompileFiles?.()) || this.projectCompile;
	}
	...
	#compileSource(): string {
		const f =
			this.#files.files.find((x) => x.id === this.#files.activeId) ?? this.#files.activeFile;
