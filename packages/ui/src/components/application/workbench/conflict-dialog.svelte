...
<Dialog open={files.pending !== null} onOpenChange={(o) => (o ? null : files.cancelPending())}>
	<DialogContent class="sm:max-w-md">
		{#if files.pending?.kind === 'conflict'}
			<DialogHeader>
				<DialogTitle>{files.pending.isFolder ? 'Folder' : 'File'} already exists</DialogTitle>
				<DialogDescription>
					{files.pending.isFolder ? 'A folder' : 'A file'} named “{files.pending.name}” already
					exists here.{files.pending.canMerge
			...
			<div class="flex flex-col gap-1.5">
				...
				<input
					...
					spellcheck="false"
					onkeydown={(e) => {
						if (e.key === 'Enter') files.resolveConflict('rename', files.conflictName);
					}}
				/>
			...
			<DialogFooter>
				<Button variant="ghost" size="sm" onclick={() => files.resolveConflict('skip')}>Skip</Button
				>
				{#if files.pending.canMerge}
					<Button variant="outline" size="sm" onclick={() => files.resolveConflict('merge')}>
						Merge
					</Button>
				{/if}
		...
		{:else if files.pending?.kind === 'confirm'}
			...
			<DialogFooter>
				<Button variant="ghost" size="sm" onclick={() => files.resolveConfirm(false)}>Cancel</Button
				>
				<Button variant="destructive" size="sm" onclick={() => files.resolveConfirm(true)}>
