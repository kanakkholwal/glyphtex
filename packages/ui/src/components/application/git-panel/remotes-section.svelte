	...
	{#if store.sections.remotes}
		<div transition:slide={{ duration: 200, easing: cubicOut }} class="mt-1 flex flex-col gap-1.5">
			...
			{#if store.addingRemote}
				<div class="border-border/60 flex flex-col gap-1 rounded border p-1.5">
					...
					<div class="flex gap-1.5">
						...
						<Button variant="ghost" size="xs" onclick={() => (store.addingRemote = false)}
							>Cancel</Button
						>
					</div>
				</div>
				...
				{#if store.editingRemote === r.name}
					<div class="border-border/60 flex flex-col gap-1 rounded border p-1.5">
						...
						<div class="flex gap-1.5">
							...
							<Button variant="ghost" size="xs" onclick={() => (store.editingRemote = null)}>
			...
			{#if store.hasRemote}
				...
				{/if}
				<div class="flex gap-1.5">
					<Button variant="outline" size="xs" disabled={store.busy} onclick={() => store.doFetch()}
						>Fetch</Button
					>
					<Button variant="outline" size="xs" disabled={store.busy} onclick={() => store.doPull()}
						>Pull</Button
					>
					<Button variant="outline" size="xs" disabled={store.busy} onclick={() => store.doPush()}
						>Push</Button
					>
				</div>
				{#if store.remoteMsg}
