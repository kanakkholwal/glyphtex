<script lang="ts" module>
	...
	/** Glyph fill: `mono` (currentColor), brand `gradient`, or fixed `white` / `dark` for coloured surfaces. */
...
<script lang="ts">
	...
	// Mono by default: blue is the one signal colour, and a gradient mark in chrome competes with it.
	let {
		text = true,
		badge = true,
		size = 'md',
		...
		/** `gradient` is opt-in, for surfaces where the mark is the subject rather than chrome. */
		tone?: LogoTone;
		href?: string;
...
{#snippet glyph(s: number, fill: string)}
	...
	>
		...
		<defs>
			...
			>
				<stop style:stop-color="color-mix(in oklch, var(--primary), white 20%)" />
				<stop offset="0.5" style:stop-color="var(--primary)" />
				<stop offset="1" style:stop-color="var(--primary-active)" />
			</linearGradient>
		</defs>
	</svg>
{/snippet}
...
>
	{#if badge}
		<span
			class="bg-action text-action-foreground grid shrink-0 place-items-center rounded-[28%]"
			style:width={`${px}px`}
			style:height={`${px}px`}
		>
	...
	{#if text}
		<span class={cn('font-display font-semibold leading-none tracking-tight', textCls)}
			>GlyphTeX</span
		>
	{/if}
</svelte:element>
